...
$downloadedPath = Join-Path $VaultPath '00-Inbox\Downloaded'
$cleanedPath = Join-Path $VaultPath '00-Inbox\Cleaned'
$researchPath = Join-Path $VaultPath '04-Research'
$taxonomyPath = Join-Path $VaultPath '98-AI-Context\Research Taxonomy.json'
$graphScript = Join-Path $PSScriptRoot 'Update-Knowledge-Graph.ps1'
...
function ConvertTo-SafeFileName {
    ...
    if (-not $safe) { $safe = 'Untitled Research' }
    return $safe
}
...
if (-not (Test-Path -LiteralPath $taxonomyPath)) {
    throw "Taxonomy configuration not found: $taxonomyPath"
}
...
if (-not $sourceFiles) {
    Write-Output 'No Markdown files found in 00-Inbox/Downloaded.'
    exit 0
}
...
foreach ($file in $sourceFiles) {
    ...
    foreach ($topic in $topicMatches) {
        $hubLines += '- [[04-Research/Topic-Hubs/' + $topic.hub + '|' + $topic.hub + ']]'
    }
    if (-not $hubLines) { $hubLines = @('- No topic match yet; triage manually.') }
    ...
    $output = @(
        ...
        '> The source file remains unchanged in `00-Inbox/Downloaded`. This copy only normalizes Markdown structure and line breaks; it does not summarize or remove source content.'
        ''
        '## Topic Hubs'
        ''
        $hubLines
